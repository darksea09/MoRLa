function [] = S_AltiFunc_IS2(path, DamName, Lv_range, Area, First_filter, Second_filter, Third_filter, low_con, Fourth_filter, runoff_rate, output, RGpath, shppath, numWorkers)
...
epoch = datetime(2018,1,1,0,0,0,'TimeZone','UTC');
...
folders = {'gt1l', 'gt1r', 'gt2l', 'gt2r', 'gt3l', 'gt3r'};
...
lon = h5read(input_file, ['/' folder '/sseg_mean_lon']);
lat = h5read(input_file, ['/' folder '/sseg_mean_lat']);
%WSH = double(h5read(input_file, ['/' folder '/ht_water_surf']));
%Geoid = double(h5read(input_file, ['/' folder '/segment_geoid']));
...
if isempty(x_in)
   clear('lon', 'lat');
    ...
    time = h5read(input_file, ['/' folder '/sseg_mean_time']);
    OSH = double(h5read(input_file, ['/' folder '/ht_ortho']));
    
    %W_Type = h5read(input_file, ['/' folder '/inland_water_body_type']);
    %W_Size = h5read(input_file, ['/' folder '/inland_water_body_size']);
    %W_source = h5read(input_file, ['/' folder '/inland_water_body_source']);
    %W_ID = h5read(input_file, ['/' folder '/inland_water_body_id']);
    ...
    %W_ID = W_ID(~idx);
    Set = table(time, lon, lat, OSH);
    Find_Set = find(ismember(Set.lon, x_in));
    Sel_Set = Set(Find_Set,:);
    Sel_Set_FL = [Sel_Set_FL; Sel_Set];
...
time_KOR = seconds(Sel_Set_FL.time) + epoch + hours(9);
Sel_Set_FL = table(time_KOR(:,1), time_KOR(:,1), Sel_Set_FL.lon, Sel_Set_FL.lat, Sel_Set_FL.OSH);
Sel_Set_FL.Properties.VariableNames = {'Date', 'Date_sec', 'lon', 'lat', 'OSH'};

save(fullfile(output, [output 'IS2_f0.mat']));
...
if strcmpi(First_filter, 'Y')
    ...
    idx = Sel_Set_FL.OSH < min(Lv_range) | Sel_Set_FL.OSH > max(Lv_range);    
    Sel_Set_FL = Sel_Set_FL(~idx, :);
    idx = isnan(Sel_Set_FL.OSH);
    Sel_Set_FL = Sel_Set_FL(~idx, :);
    ...
    save(fullfile(output, [output 'IS2_f1.mat']));
...
[~, p_value_left, ~, stats_left] = ttest2(Sel_Set_Dy.OSH, Sel_Set_cur_FL.OSH, 'tail', 'left');
[~, p_value_right, ~, stats_right] = ttest2(Sel_Set_Dy.OSH, Sel_Set_cur_FL.OSH, 'tail', 'right');
[~, p_value_both, ~, stats_both] = ttest2(Sel_Set_Dy.OSH, Sel_Set_cur_FL.OSH, 'tail', 'both');
...
for v = 1 : numel(DyWMs)
        ...
        if DyWM == 'FL'
            ...
            %Result_OSH = DyWM_Sel_Set.OSH - Real_WL.Var2;
            STDV_track = std(DyWM_Sel_Set.OSH);
            CI = STDV_track / sqrt(length(DyWM_Sel_Set.OSH));
            CI = repmat(CI, size(DyWM_Sel_Set.OSH));

            Result = table(DyWM_Sel_Set{:,1}, DyWM_Sel_Set{:,2}, DyWM_Sel_Set{:,3}, DyWM_Sel_Set{:,4}, DyWM_Sel_Set{:,5}, CI);
            Result.Properties.VariableNames = {'Date', 'Date_sec', 'lon', 'lat', 'OSH', 'CI'};
            
            Result_al = [Result_al; Result];
            %writetable(Result, fullfile(output, sprintf('IS2_%s.xls', varName3)), 'fileType', 'spreadsheet');

            %OSH_min = min(Result_OSH);
            %OSH_max = max(Result_OSH);
            %count_50 = sum(abs(Result_OSH) <= 0.3);
            %waterlevel = Result.gauged_WL(1,1);
            
            %DyWM_date_str = datestr(DyWM_date, 'yy-mm-dd'); % Or any format you like
            %T = table({DyWM_date_str}, size(DyWM_Sel_Set, 1), OSH_min, OSH_max, count_50, waterlevel, 'VariableNames', {'Time', 'Count', 'Min', 'Max', 'Count_50', 'waterlevel'});
            %T2 = [T2; T];
end

%writetable(T2, Sum_filename);

    idx = isnan(Result_al.OSH);
    Result_al = Result_al(~idx, :);

%    WL_Alt_al = Result_al.OSH;
%    WL_Real_al = Result_al.gauged_WL;

save(fullfile(output, [output 'IS2_f2.mat']));
end
...
%    end
    ...
    for i = 1:height(groupedTable)
        ...
        STDV_track = std(DyWM_Sel_Set.OSH(groupIndices));
    ...
    Result_al.Properties.VariableNames = {'Date', 'Date_sec', 'lon', 'lat', 'OSH', 'CI'};
    
    %Result_al = table(DyWM_Sel_Set{:,1}, DyWM_Sel_Set{:,2}, DyWM_Sel_Set{:,3}, DyWM_Sel_Set{:,4}, DyWM_Sel_Set{:,5}, Result_OSH, Real_WL.Var1, Real_WL.Var2);
    %Result_al.Properties.VariableNames = {'Date', 'Date_sec', 'lon', 'lat', 'WL_OSH', 'WL_OSH_WL', 'Date_gauged', 'gauged_WL'};
...
while h < size(groupedTable3, 1) - 1
    ...
    if groupedTable3.median_OSH(h+1) > groupedTable3.median_OSH(h)
        
        BefLv = groupedTable3.median_OSH(h);
        PreLv = groupedTable3.median_OSH(h+1);
...
save(fullfile(output, [output 'IS2_f4.mat']));
...
%% The Result
	...
	%Stat_filename = ['!Stat_IS2' output '.csv'];
    %Dam = {DamName};
    ...
    Result_Graph = table(SatName, groupedTable3.Date, groupedTable3.GroupCount, groupedTable3.median_OSH, groupedTable4.median_CI, ...
    'VariableNames', {'SatName', 'Date', 'GroupCount', 'median_OHS', 'median_CI'});

    assignin('base', 'Result_IS2', Result_Graph);
    
    save(fullfile(output, [output 'IS2_ff.mat']));

function [] = S_AltiFunc_S3A(path, DamName, Lv_range, Area, First_filter, Second_filter, Third_filter, low_con, Fourth_filter, runoff_rate, output, RGpath, shppath, numWorkers)
...
files = dir(fullfile(path, '*S3A*.nc'));
filename = {files.name};
...
save(fullfile(output, [output 'SEN3A_f0.mat']));
...
%% Filter 1 : Standard Operation Level (No option - Mandatory)
    ...
    save(fullfile(output, [output 'SEN3A_f1.mat']));
...
save(fullfile(output, [output 'SEN3A_f2.mat']));
end
...
save(fullfile(output, [output 'SEN3A_f3.mat']));
...
save(fullfile(output, [output 'SEN3A_f4.mat']));
...
%% The Result
	...
	%Stat_filename = ['!Stat_S3A' output '.csv'];
    %Dam = {DamName};
    ...
    assignin('base', 'Result_S3A', Result_Graph);
                 
    save(fullfile(output, [output 'SEN3A_ff.mat']));
    
%    clear Table_S_Dy1 del_groupedTable3 Result_al T2 DyWMs Sel_Set_FL t_test* Sel_Set_* Close_Time Real_WL

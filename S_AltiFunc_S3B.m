function [] = S_AltiFunc_S3B(path, DamName, Lv_range, Area, First_filter, Second_filter, Third_filter, low_con, Fourth_filter, runoff_rate, output, RGpath, shppath, numWorkers)
...
files = dir(fullfile(path, '*S3B*.nc'));
filename = {files.name};
...
y_in = lat(in);
    ...
    %W_ID = W_ID(~idx);
        Set = table(time, lon, lat, alt, range_ocog, range_ocean, iono_cor);
    ...
    %latHZ1=ncread(input_file,'lat_01');
        ...
        mod_dry_tropo_cor_01=ncread(input_file,'mod_dry_tropo_cor_meas_altitude_01');
        sel_dry_tropo_cor = interp1(lonHZ1, mod_dry_tropo_cor_01, Sel_Set.lon);
        mod_wet_tropo_cor_01=ncread(input_file,'mod_wet_tropo_cor_meas_altitude_01');
        ...
        Result_ocog = (Sel_Set.alt - Sel_Set.range_ocog)*1000 - sel_Geoid - sel_dry_tropo_cor - sel_wet_tropo_cor - sel_pole_tide_cor - sel_earth_tide_cor - Sel_Set.iono_cor;
        Result_ocean = (Sel_Set.alt - Sel_Set.range_ocean)*1000 - sel_Geoid - sel_dry_tropo_cor - sel_wet_tropo_cor - sel_pole_tide_cor - sel_earth_tide_cor - Sel_Set.iono_cor;
    
        Sel_Set2 = table(Sel_Set.time, Sel_Set.lon, Sel_Set.lat, Result_ocog, Result_ocean);
...
Sel_Set_FL = table(time_KOR(:,1), time_KOR(:,1), Sel_Set_FL.Var2, Sel_Set_FL.Var3, Sel_Set_FL.Var4, Sel_Set_FL.Var5);
Sel_Set_FL.Properties.VariableNames = {'Date', 'Date_sec', 'lon', 'lat', 'WL_ocog', 'WL_ocean'};

save(fullfile(output, [output 'SEN3B_f0.mat']));
...
%% Filter 1 : Standard Operation Level (No option - Mandatory)
    ...
    if strcmpi(First_filter, 'Y')
        ...
        idx = Sel_Set_FL.WL_ocog < min(Lv_range) | Sel_Set_FL.WL_ocog > max(Lv_range);    
        Sel_Set_FL = Sel_Set_FL(~idx, :);
        idx = isnan(Sel_Set_FL.WL_ocog);
        Sel_Set_FL = Sel_Set_FL(~idx, :);
    ...
    save(fullfile(output, [output 'SEN3B_f1.mat']));
...
for e = 1:height(groupedTable)
    ...
    %for w = 1 : interval : numel(WaterMasks)
...
%varName1 = ['Sel_Set_Dy_', datestr(current_group_date, 'yy_mm_dd'), '_', num2str(WaterMask)];
%assignin('base', varName1, Sel_Set_Dy);
Sel_Set_end{w} = Sel_Set_Dy;

[~, p_value_left, ~, stats_left] = ttest2(Sel_Set_Dy.WL_ocog, Sel_Set_cur_FL.WL_ocog, 'tail', 'left');
[~, p_value_right, ~, stats_right] = ttest2(Sel_Set_Dy.WL_ocog, Sel_Set_cur_FL.WL_ocog, 'tail', 'right');
[~, p_value_both, ~, stats_both] = ttest2(Sel_Set_Dy.WL_ocog, Sel_Set_cur_FL.WL_ocog, 'tail', 'both');

%t_test = [t_test; WaterMask, p_value_left, stats_left.tstat, stats_left.df, p_value_right, stats_right.tstat, stats_right.df, p_value_both, stats_both.tstat, stats_both.df];
...
minValue = min(abs(eval([varName2 '.p_value_right'])));
...
if  isnan(minValue)
    ...
    mVIndex = eval([varName2 '.p_value_right == minValue']);
    %assignin('base', 'mVIndex', mVIndex);
    DSel_WM = min(eval([varName2 '{mVIndex, 1}']));
    %DSel_WM = min(evalin('base', [varName2 '{' num2str(mVIndex) ', 1}']));
end
...
for v = 1 : numel(DyWMs)
        ...
        if DyWM == 'FL'
            ...
            %Result_OHS = DyWM_Sel_Set.WL_ocog - Real_WL.Var2;
            STDV_track = std(DyWM_Sel_Set.WL_ocog);
            CI = STDV_track / sqrt(length(DyWM_Sel_Set.WL_ocog));
            CI = repmat(CI, size(DyWM_Sel_Set.WL_ocog));

            Result = table(DyWM_Sel_Set{:,1}, DyWM_Sel_Set{:,2}, DyWM_Sel_Set{:,3}, DyWM_Sel_Set{:,4}, DyWM_Sel_Set{:,5}, CI);
            Result.Properties.VariableNames = {'Date', 'Date_sec', 'lon', 'lat', 'WL_ocog', 'CI'};
...
%save 'Result_al.mat' Result_al     
%disp(Result_al)
    
    idx = isnan(Result_al.WL_ocog);
    Result_al = Result_al(~idx, :);

%    WL_Alt_al = Result_al.WL_ocog;
%    WL_Real_al = Result_al.gauged_WL;

save(fullfile(output, [output 'SEN3B_f2.mat']));
end
...
if strcmpi(Second_filter, 'N')
    ...
    for i = 1:height(groupedTable)
        ...
        STDV_track = std(DyWM_Sel_Set.WL_ocog(groupIndices));
    ...
    Result_al.Properties.VariableNames = {'Date', 'Date_sec', 'lon', 'lat', 'WL_ocog', 'CI'};
...
while h < size(groupedTable3, 1) - 1
    ...
    if groupedTable3.median_WL_ocog(h+1) > groupedTable3.median_WL_ocog(h)
        
        BefLv = groupedTable3.median_WL_ocog(h);
        PreLv = groupedTable3.median_WL_ocog(h+1);
...
save(fullfile(output, [output 'SEN3B_f4.mat']));
...
%% The Result
	...
	%Stat_filename = ['!Stat_S3B' output '.csv'];
    %Dam = {DamName};
    ...
    Result_Graph = table(SatName, groupedTable3.Date, groupedTable3.GroupCount, groupedTable3.median_WL_ocog, groupedTable4.median_CI, ...
                     'VariableNames', {'SatName', 'Date', 'GroupCount', 'median_WL_ocog', 'median_CI'});

    assignin('base', 'Result_S3B', Result_Graph);
                 
    save(fullfile(output, [output 'SEN3B_ff.mat']));
    
%    clear Table_S_Dy1 del_groupedTable3 Result_al T2 DyWMs Sel_Set_FL Passfilename t_test* Sel_Set_* Real_WL
